...
subsample.df$tool[subsample.df$tool == "cgt"] <- "Adjusted"
...
subsample.df$tool <- factor(subsample.df$tool, levels = c("Adjusted", "Unadjusted"))

if (to_analyse == "rare")
{
  subsample.df.rare <- subset(subsample.df, variable == "rare")
  subsample.df.rare$variable <-"Number of rare genes"
  
  rare_p <- ggplot(data=subsample.df.rare, aes(x=rare_lim, y=value, colour=type)) + facet_grid(stringency~tool, scales = "free_y") + geom_point(size=2, alpha=0.5) + geom_line(alpha=0.5) + theme_light() + xlab("Rare threshold") + ylab("Number of rare genes") + theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 14), strip.text.y = element_text(size = 14), legend.title=element_text(size=18,face="bold"), legend.text=element_text(size=16)) + guides(colour=guide_legend(title="Simulation type")) + scale_colour_npg() + scale_y_continuous(breaks = seq(0, 100000, 2000))
  rare_p
  write.csv(total.df, file='panaroo_rare.csv')
} else if (to_analyse == "core")
{
  subsample.df.core <- subset(subsample.df, variable == "core")
  subsample.df.core$variable <-"Number of core genes"
  
  core_p <- ggplot(data=subsample.df.core, aes(x=core_lim, y=value, colour=type)) + facet_grid(stringency~tool, scales = "free_y") + geom_point(size=2, alpha=0.5) + geom_line(alpha=0.5) + theme_light() + xlab("Core threshold") + ylab("Number of core genes") + theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 14), strip.text.y = element_text(size = 14), legend.title=element_text(size=18,face="bold"), legend.text=element_text(size=16)) + guides(colour=guide_legend(title="Simulation type")) + scale_colour_npg() + scale_y_continuous(breaks = seq(0, 100000, 2000))
  core_p
  write.csv(total.df, file='panaroo_core.csv')
}

grid.plot <- ggarrange(core_p, rare_p, align="hv", ncol = 2, common.legend = TRUE, legend="right", labels="AUTO")
grid.plot

ggsave(file="Panaroo_simulation_figure.png", plot=grid.plot, width=16, height=7)